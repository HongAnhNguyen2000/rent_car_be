...
import { getBookings } from '_redux/slices/booking';
// utils
...
// @types
import { Car, CarState } from '@types/bookings';
// components
...
import {
  BookingListHead,
  BookingListToolbar
} from 'components/_dashboard/booking/list';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'booking', label: 'Car', alignCenter: true },
...
function applySortFilter(array: Car[], comparator: (a: any, b: any) => number, query: string) {
  ...
  if (query) {
    return filter(
      array,
      (_booking) => _booking.title.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
...
export default function BookingList() {
  ...
  const { bookings } = useSelector((state: { booking: CarState }) => state.booking);
  ...
  const handleSelectAllClick = (checked: boolean) => {
    if (checked) {
      const selected = bookings.map((n) => n.id);
  ...
  const filteredCars = applySortFilter(bookings, getComparator(order, orderBy), filterTitle);

  const isCartNotFound = filteredCars.length === 0;

  return (
    <Page title="Evs rental cms: List booking">
      <Container>
        <HeaderBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Booking',
              href: PATH_DASHBOARD.booking.root
            },
            { name: 'List' }
          ]}
        />
